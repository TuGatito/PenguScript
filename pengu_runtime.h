...
extern "C"
{
  ...
  /**
   * @brief Checks if all characters in string s are alphabetic.
   * @param s String to test.
   * @return True if non-empty and all alphabetic, false otherwise.
   */
  ...
  /**
   * @brief Checks if all characters in string s are decimal digits.
   * @param s String to test.
   * @return True if non-empty and all numeric digits, false otherwise.
   */
  ...
  /**
   * @brief Checks if all characters in string s are alphanumeric.
   * @param s String to test.
   * @return True if non-empty and all alphanumeric, false otherwise.
   */
...
#define PENGU_ARRAY_TYPE(T, N) \
  ...
  /** @brief Checks if integer is a prime number. */
  ...
  /** @brief Computes greatest common divisor using Euclidean algorithm. */
  ...
  /** @brief Computes least common multiple. */
  ...
  /* =========================================================================
   * 15. Extended String Manipulation (Scrolls)
   * ========================================================================= */
  ...
  {
    if (!s.data || !prefix.data || prefix.len > s.len)
      return false;
  ...
  {
    if (!s.data || !suffix.data || suffix.len > s.len)
      return false;
  ...
  {
    if (!s.data || !sub.data || sub.len > s.len)
      return -1;
  ...
  {
    if (!s.data || start < 0 || start >= s.len || end_pos <= start)
      return pengu_string_from_cstr("");
    if (end_pos > s.len)
      end_pos = s.len;
  ...
  static inline PenguString scrolls_replace(PenguString s, PenguString from_str, PenguString to_str)
  {
    ...
    pengu_banish_string(&p1);
    pengu_banish_string(&p2);
    return res;
  }
  ...
  static inline PenguString scrolls_join(PenguList parts, PenguString delim)
  {
    ...
    for (int i = 1; i < parts.len; ++i)
    {
      ...
      if (delim.len > 0)
        res = pengu_string_concat(res, delim);
      if (p)
        res = pengu_string_concat(res, *p);
    }
    return res;
  }
  static inline PenguString scrolls_repeat(PenguString s, int count)
  {
    if (count <= 0 || !s.data || s.len == 0)
      return pengu_string_from_cstr("");
  ...
  /* =========================================================================
   * 16. File System Path Manipulation (Compass)
   * ========================================================================= */
  ...
  static inline bool pengu_c_path_is_absolute(PenguString path)
  {
    ...
    {
      if (path.len >= 3 && pengu_c_path_is_sep(path.data[2]))
        return true;
      return true;
    }
    if (pengu_c_path_is_sep(path.data[0]))
      return true;
    return false;
#else
  return path.data[0] == '/';
  ...
  static inline bool pengu_c_path_is_root(PenguString path)
  {
    if (!path.data || path.len == 0)
      return false;
    if (path.len == 1 && pengu_c_path_is_sep(path.data[0]))
      return true;
#if PENGU_WINDOWS
    if (path.len == 2 && isalpha((unsigned char)path.data[0]) && path.data[1] == ':')
      return true;
    if (path.len == 3 && isalpha((unsigned char)path.data[0]) && path.data[1] == ':' && pengu_c_path_is_sep(path.data[2]))
      return true;
#endif
    return false;
  }
  ...
  static inline PenguString pengu_c_path_basename(PenguString path)
  {
    ...
    if (end == 0 && pengu_c_path_is_sep(path.data[0]))
      return pengu_string_from_cstr("");
    int start = end;
    while (start >= 0 && !pengu_c_path_is_sep(path.data[start]))
    {
#if PENGU_WINDOWS
      if (start == 1 && path.data[start] == ':' && isalpha((unsigned char)path.data[0]))
        break;
#endif
      start--;
    }
  ...
  static inline PenguString pengu_c_path_dirname(PenguString path)
  {
    ...
    {
#if PENGU_WINDOWS
      if (end == 1 && path.data[end] == ':' && isalpha((unsigned char)path.data[0]))
        break;
#endif
      end--;
    }
    ...
    {
#if PENGU_WINDOWS
      if (end == 2 && path.data[1] == ':' && isalpha((unsigned char)path.data[0]))
        break;
#endif
      end--;
    }
  ...
  {
    if (pengu_c_path_has_ext(path) || ext.len == 0)
      return path;
    char buf[4096];
  ...
  static inline PenguMaybe pengu_c_path_relative_to(PenguString target, PenguString base)
  {
    ...
    if (t_parts.len == 0 || b_parts.len == 0)
      return pengu_maybe_none();
    ...
    while (common < t_parts.len && common < b_parts.len)
    {
      ...
      if (p1->len != p2->len || memcmp(p1->data, p2->data, (size_t)p1->len) != 0)
        break;
      common++;
    }
    if (common == 0 && pengu_c_path_is_absolute(norm_target))
      return pengu_maybe_none();
  ...
  static inline PenguMaybe pengu_c_archivum_read_lines(PenguString path)
  {
    ...
    *lines = pengu_list_new(sizeof(PenguString), 16);
    ...
    for (int i = 0; i <= content->len; ++i)
    {
      ...
      {
        int end = i;
        if (end > start && content->data[end - 1] == '\r')
          end--;
        int len = end - start;
        ...
        {
          if (len > 0)
            memcpy(line_buf, content->data + start, (size_t)len);
          line_buf[len] = '\0';
  ...
  /** @brief Encodes binary or text string to Base64. */
  ...
  /** @brief Decodes Base64 string to original payload. */
  static inline PenguMaybe pengu_c_cipher_decode_base64(PenguString data)
  {
    ...
    for (int i = 0; i < clen; i += 4)
    {
      ...
      if (clean[i + 2] != '=' && j < out_len)
        out[j++] = (char)((triple >> 8) & 0xFF);
      if (clean[i + 3] != '=' && j < out_len)
  ...
  {
    while (*idx < len && isspace((unsigned char)s[*idx]))
      (*idx)++;
  }
  ...
  /** @brief Parses JSON string into a PenguMap of key-value strings. */
  static inline PenguMaybe pengu_c_cipher_parse_json(PenguString json)
  {
    ...
    *map = pengu_map_new(sizeof(PenguString), sizeof(PenguString));
    ...
    while (idx < json.len)
    {
      ...
      if (idx < json.len && json.data[idx] == ',')
        idx++;
      else if (idx < json.len && json.data[idx] == '}')
      {
        idx++;
        break;
      }
  ...
  /** @brief Serializes a PenguMap to a JSON string. */
  ...
  static inline bool pengu_c_ledger_write_file(PenguString path, PenguList rows, PenguString delimiter)
  {
    ...
    if (!m_csv.is_present || !m_csv.value)
      return false;
