...
/* C99 bool for uuid_from_string() (the header uses `bool` without including
 * <stdbool.h> itself; C++ gets bool as a builtin). */
#include <stdbool.h>
...
/* PenguScript vendoring fix: the upstream header uses `uuid` (not
 * `struct uuid`) throughout its prototypes but never typedef'd the struct,
 * which only compiles in C++ (where a struct tag is a type name). This
 * typedef makes the header valid C99/C11 as well. */
...
/* MinGW / MinGW-w64: BCrypt RNG used by the uuid4_generate() port below. */
#  include <windows.h>
#  include <bcrypt.h>
#endif
...
void uuid4_generate( uuid* res )
{
	...
	CFRelease ( new_uuid );
#elif defined(_WIN32)
	/* MinGW / MinGW-w64 port (added by PenguScript): generate a random
	 * version-4 UUID straight into res->bytes using the BCrypt RNG.
	 * BCryptOpenAlgorithmProvider/BCryptGenRandom return NTSTATUS which is
	 * 0 (STATUS_SUCCESS) on success; we only need to test against 0. */
	{
		BCRYPT_ALG_HANDLE alg = NULL;
		if (BCryptOpenAlgorithmProvider( &alg, BCRYPT_RNG_ALGORITHM, NULL, 0 ) != 0)
			return;
		if (BCryptGenRandom( alg, res->bytes, 16, 0 ) != 0)
		{
			BCryptCloseAlgorithmProvider( alg, 0 );
			return;
		}
		BCryptCloseAlgorithmProvider( alg, 0 );
		res->bytes[6] = (unsigned char)((res->bytes[6] & 0x0F) | 0x40); /* version 4 */
		res->bytes[8] = (unsigned char)((res->bytes[8] & 0x3F) | 0x80); /* RFC 4122 variant */
	}
#else
